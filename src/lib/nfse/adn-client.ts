...
export async function fetchDanfse(
  ...
  return new Promise((resolve, reject) => {
    const req = https.request(adnUrl, { method: "GET", agent, headers: { Accept: "application/pdf" }, timeout: 15000 }, (res) => {
      const chunks: Buffer[] = []
      res.on("data", (chunk: Buffer) => chunks.push(chunk))
      res.on("end", () => {
        const buffer = Buffer.concat(chunks)
        if (res.statusCode && res.statusCode >= 400) {
          reject(new Error(`ADN DANFSE ${res.statusCode}. Baixe manualmente em www.nfse.gov.br (Consulta Publica > chave: ${chaveAcesso})`))
          return
        }
        resolve(buffer)
      })
    ...
    req.on("error", reject)
    req.on("timeout", () => { req.destroy(); reject(new Error("ADN timeout")) })
    req.end()
  })
}
